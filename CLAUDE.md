...
0. **Cross-audyt**: kto napisał specyfikację, ten jej nie audytuje. Etykietę `by:agent`
   ...
   **Podpisuj każdą treść issue i każdy komentarz**, który piszesz — ostatnią linijką:
   ...
   oznaczanie selektywne degraduje się do braku oznaczeń. Konwencja obowiązuje od
   2026-09-08; wcześniejsze wpisy są niepodpisane, a późniejsze bez podpisu pochodzą
   od właściciela. Etykieta `by:agent` zostaje mimo to, bo działa na listach i filtrach,
   których podpis nie obsługuje.
