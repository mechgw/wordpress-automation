...
function testGA4() {
  ...
  if (!cfg.propertyId) {
    const matching = enriched.filter(p =>
      p.streamUrls.some(url => /(^|\.)citycouriers\.pl/i.test(extractHostname_(url)))
    );
    ...
    cfgSheet.getRange('B9').setValue(
      !properties.length
        ...
        : cityMatches.length > 1
          ? 'KILKA WŁAŚCIWOŚCI MA STRUMIEŃ CITYCOURIERS.PL – WYBIERZ PROPERTY ID Z LISTY E:H'
...
/**
 * Ustawia właściwe key events dla CityCouriers:
...
function ustawAutomatycznyImportGA4() {
  ...
  SpreadsheetApp.getActive()
    .getSheetByName(GA4_CONFIG_SHEET)
    .getRange('B9')
    .setValue('AUTOMATYCZNY IMPORT WŁĄCZONY – około 06:00');
}
...
function importGa4Range_(startDate, endDate, cfg) {
  ...
  SpreadsheetApp.getActive()
    ...
    .setValue(
      ...
      Utilities.formatDate(importedAt, GA4_TZ, 'yyyy-MM-dd HH:mm') +
      ' | landing: ' + landingRows.length +
      ' | key events: ' + eventRows.length +
      ' | business: ' + businessRows.length +
      ' | ads: ' + adsRows.length +
      (adsWarning ? ' | UWAGA ADS: ' + adsWarning : '')
    );
}
