...
describe('#69: strażnik z menu pokazuje wynik', () => {
  ...
  test('menu Dane wskazuje wariant z oknem, trigger nadal na funkcję bez okna', () => {
    ...
    assert.deepEqual(dane.items.map(i => i.fn), ['showImportStatus', 'refreshImportStatusCells', 'sprawdzAktualnoscImportowZMenu', 'ustawCodzienneAlerty', 'diagnostykaSystemu', 'pokazZajetoscArkusza', 'wyczyscStareSnapshotyIWyniki', 'przytnijPusteWiersze', 'uporzadkujArkuszeZMenu', 'ukryjArkuszeTechniczne', 'pokazArkuszeTechniczne']);
    gas.ustawCodzienneAlerty();
