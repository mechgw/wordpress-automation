...
function changedLines(mode) {
  ...
  // i wymagałoby 100% pokrycia każdej istniejącej linii.
  const diffArgs = ['diff', '--no-color', '-U0', '-M'];
  if (mode === 'staged') diffArgs.push('--cached');
  else diffArgs.push(`${mode.slice('base:'.length)}...HEAD`);
  // Pathspec obejmuje też katalog główny: gdyby ograniczyć go do src/, git nie
  // zobaczyłby strony źródłowej przeniesienia i nie rozpoznałby zmiany nazwy.
  // Wyniki spoza src/ odfiltrowujemy niżej.
  diffArgs.push('--', '*.gs');
...
function main() {
  ...
  const sources = fs.readdirSync(path.join(ROOT, SOURCE_DIR)).filter(f => SOURCE_GLOB.test(f)).map(f => SOURCE_DIR + '/' + f).sort();
