...
Osobnym i zwykle większym problemem jest pusty przydział. Google liczy do limitu całą siatkę, nie wypełnione komórki, więc zakładka z kilkoma tysiącami wierszy danych rozciągnięta na sto tysięcy wierszy zajmuje kilkanaście razy więcej, niż powinna. *Dane → Przytnij puste wiersze* usuwa wyłącznie wiersze poniżej ostatniego wypełnionego, zostawiając dwa tysiące wolnych na zapas, i pokazuje przed usunięciem, co dokładnie zniknie. Granica jest wyznaczana przez skan wartości i formuł od dołu, a nie przez `getLastRow()`: ten zwraca koniec zakresu danych arkusza, który bywa zawyżony przez samo formatowanie po dawnych importach. Formuła licząca się do pustego tekstu też jest treścią i nie zostanie usunięta. Żadna dana przy tym nie ginie, ale formuła odwołująca się wprost do komórki z usuwanego zakresu pokaże potem #REF!, i dialog o tym uprzedza.
